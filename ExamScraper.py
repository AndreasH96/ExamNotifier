...
class ExamScraper:
    ...
    def updateHTMLSource(self):
        self.hhAvailableExamsSourceHTML = urllib.request.urlopen(
            "https://www.hh.se/student/innehall-a-o/tentor-att-hamta-ut.html"
        ).read()
    ...
    def updateExamDataFrame(self):
        ...
        for caption in self.soup.find_all("caption"):
            for academy in self.academys:
                ...
                if academy in caption.get_text() and academy != "ETN":
                    ...
                    examTableHTML = caption.find_parent(
                        "table",
                        attrs={
                            "class": "sv-responsiveTable sv-responsiveTable--stacked sv-responsiveTable--stackTable c6"
                        },
                    )
                    examTableRowsHTML[academy] = examTableHTML.find_all("tr")

                elif academy == "ETN" and examTableRowsHTML["ETN"] == None:
                    # print(academy)
                    # print(caption.get_text())
                    examTableHTML = self.soup.find(
                        "table",
                        attrs={
                            "class": "sv-responsiveTable sv-responsiveTable--stacked sv-responsiveTable--stackTable c6"
                        },
        ...
        for academy in exams:
            ...
            self.examDataFrame[academy] = pd.DataFrame(
                exams[academy], columns=["Date", "Course_Code", "Title", "Examinator"]
            )
